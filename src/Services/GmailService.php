...
class GmailService
{
    ...
    public static function display(DMessage $dm): DMessage
    {
        ...
        if (!empty($dm->attachments)) {
            ...
            foreach ($attachments as $a) {
                ...
                $p = '';
                if ($class) {
                    $p = $class->getMethod('getMsgPath')->invoke(null, $a);
                }
                if (in_array($e, DAttachment::IMG_EXT) && $cid) {
                    $images[$cid] = $p;
                } else {
                    $html .= "<a href='{$p}' target='_blank' download='{$fn}'>{$fn}</a><br/>";
                }
            }
            /** @var string $content */
            $content = preg_replace_callback($pregRule, function ($matches) use ($images) {
     ...
     */
    public static function getMsg($gid, Carbon $since, $cf, $cacheContent, $maxResults = 0): \Generator|array
    {
        $nextToken = 1;
        $cgFlag = 0;
        while ($nextToken) {
            $results = static::$service->users_messages
                ->listUsersMessages(static::$user, [
                    'pageToken'  => $nextToken,
                    'maxResults' => DHelper::getNoEmpty($maxResults, static::$config['max_results'], 10),
                    'q'          => 'after:' . $since->rawFormat("Y/m/d")
                ]);
     ...
     * @return \Generator|DMessage[]
     */
    public static function syncMail($maxResults = 0): \Generator|array
    {
        ...
        return static::getMsg($gid, $since, $cf, $cacheContent, $maxResults);
    }
